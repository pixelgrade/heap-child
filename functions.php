<?php
/**
 * Heap Child functions and definitions
 *
...
/**
 * Setup Heap Child Theme's textdomain.
 *
 * Declare textdomain for this child theme.
 * Translations can be filed in the /languages/ directory.
 */
function heap_child_theme_setup() {
	load_child_theme_textdomain( 'heap-child', get_stylesheet_directory() . '/languages' );
}
add_action( 'after_setup_theme', 'heap_child_theme_setup' );
...
add_action( 'wp_enqueue_scripts', 'heap_child_enqueue_styles' );
...
function heap_child_overwrite_files() {
	...
	// See the parent's function.php -> the heap_scripts_styles() function
	// for details like resources names

		wp_dequeue_script( 'heap-main-scripts' );
	...
	// This is not required, but I assume you are not modifying that much :)
		...
		wp_enqueue_script( 'heap-child-scripts',
	...
	// see the parent's function.php -> the hive_scripts_styles() function for details like resources names
		...
		wp_dequeue_style( 'heap-main-style' );
...
// Load the files from the function mentioned above:
	...
	add_action( 'wp_enqueue_scripts', 'heap_child_overwrite_files', 11 );
